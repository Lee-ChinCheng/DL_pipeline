...
class Evaluater(tf.keras.callbacks.Callback):
    ...
    def __init__(self, prefix, generator, steps, batch_size, verbose=1): #initial verbose=1
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        #stat = [float(f"{s:.3f}") for s in stat]
        #logs.update({
        #    f"{self.prefix}_{self.model.metrics_names[i]}": s
        ...
        # Custom print
        #msg = " - ".join(
        #    [f"{self.prefix}_{self.model.metrics_names[i]}: {stat[i]:.3f}"
        #    for i in range(len(stat))]
        #)
        #print(f"\nEpoch {epoch+1} {msg}")
        #Epoch 80 val_loss_dropout: 0.581 - val_accuracy_dropout: 0.759 - val_loss: 0.581 - val_accuracy: 0.759 - 
        # val_specificity: 0.828 - val_precision: 0.707 - val_recall: 0.651 - val_auROC: 0.806 - val_TP: 41.000 - val_FP: 17.000 - val_FN: 22.000 - val_TN: 82.000
...
class LearningRateScheduler():
    ...
    def __init__(self, epochs, initial_lr, decay_rate, bottom_lr, fold_num):

        self._initial_lr = initial_lr
        self._decay_rate = decay_rate
        self._bottom_lr = bottom_lr
        self._change_pt = math.ceil(epochs * 0.1)
    ...
    def _cosine_decay(self, epoch):
        ...
        print('\nLR: %.8f' % lr, ' change_pt, slow_pt', self._change_pt, self._slow_pt, ' fold', self._fold_num)
